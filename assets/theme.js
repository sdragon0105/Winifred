    ...
    CartDrawer.prototype = $.extend({}, CartDrawer.prototype, {
      ...
      buildCart: function(cart, openDrawer, returnFocusEl) {
        ...
        if (cart.item_count === 0) {
          ...
          /* Cart Motivator - Hafiz Adnan Hussain - START */

          // Note: all these variables are passed to "data" object
          var totalPrice = cart.total_price;
          var motivatorMin = theme.settings.motivatorMin;
          var motivatorNeed = theme.Currency.formatMoney((motivatorMin - totalPrice), theme.settings.moneyFormat);
          var motivatorQualifies = false;
          var motivatorPercent = '0';

          if (totalPrice > motivatorMin) {
            motivatorQualifies = true;
            motivatorPercent = '100';
          } else {
            motivatorPercent = ((totalPrice/1.0/motivatorMin) * 100);
          }

          /* Cart Motivator - Hafiz Adnan Hussain - END */
          ...
          data = {
            ...
            motivatorMin: motivatorMin,
            motivatorNeed: motivatorNeed,
            motivatorReward: theme.settings.motivatorReward,
            motivatorQualifies: motivatorQualifies,
            motivatorPercent: motivatorPercent
          };
      ...
      $('body, .modal__inner')
        ...
        // ---------------------------------------
          if (!isOpen) {
            ...
            $el.css("border", "hidden");
            $el.parent().children().last().css('borderBottom', '1px solid rgba(34,80,83,.45)');
            console.log($el.parent().children().last().children().first());
          } else {
            $el.css("borderBottom", "1px solid rgba(34,80,83,.45)");
            $el.parent().children().last().css("border", "none");
          }
        // ---------------------------------------
...
/* Cart Upsell - Hafiz Adnan Hussain - START */
document.addEventListener('cart:updated', function(evt) {

  //console.log(evt.detail.cart);
  var cart_items = evt.detail.cart.items;

  if(theme.settings.cartUpsell && cart_items.length > 0) {

    jQuery.getJSON("/recommendations/products.json?product_id="+parseInt(cart_items[0].product_id)+"&limit=10", function(response) {
      ...
      if (recommendedProducts.length > 0) {
        ...
        var upsel_markup = '',
            j = 0,
            cart_ids = [5518369882270];

        $.each(cart_items, function(key, item){
          cart_ids.push(item.product_id);
        });

        for (i = 0; i < recommendedProducts.length; i++) {

          if(cart_ids.includes(recommendedProducts[i].id) || recommendedProducts[i].available == false || recommendedProducts[i].variants[0].available == false) {
            continue;
          }

          upsel_markup += '<form method="post" action="/cart/add" id="product_form_'+recommendedProducts[i].id+'" accept-charset="UTF-8" class="ProductForm ProductCartUpsell" enctype="multipart/form-data" data-productid="'+recommendedProducts[i].id+'">';

            upsel_markup += '<div class="cart-item"><div class="cart-item-content" style="display:flex;">';

            upsel_markup += '<div class="cart-item__image">';
              upsel_markup += '<a href="'+recommendedProducts[i].variants[0].id+'">';
              upsel_markup += '<img src="'+recommendedProducts[i].featured_image+'" class="lazyload" alt="'+recommendedProducts[i].title+'" width="80">';
              upsel_markup += '</a>';
            upsel_markup += '</div>';

            upsel_markup += '<div class="cart-item__product">';
              upsel_markup += '<a class="cart-item__title" href="'+recommendedProducts[i].url+'">';
              upsel_markup += '<span>'+recommendedProducts[i].title+'</span>';
              //upsel_markup += '<small>'+recommendedProducts[i].variants[0].title+'</small>';
              upsel_markup += '<input type="hidden" name="id" data-productid="'+recommendedProducts[i].id+'" data-sku="'+recommendedProducts[i].variants[0].sku+'" value="'+recommendedProducts[i].variants[0].id+'">';
              upsel_markup += '</a>';
              upsel_markup += '<span class="cart-item__price">'+theme.Currency.formatMoney(recommendedProducts[i].price, theme.settings.moneyFormat)+'</span>';
            upsel_markup += '</div>';
            upsel_markup += '<div class="cart_actions">';
              upsel_markup += '<button type="submit" class="upsell-cta cart-drawer-upsell-cta action_button btn add-to-cart btn--secondary" data-product-atc data-action="add-to-cart">Add</button>';
            upsel_markup += '</div>';

            upsel_markup += '</div></div><!--  /.cart-item  -->';

            upsel_markup += '<input type="hidden" name="quantity" value="1">';
        ...
        $('.sidebarcart-upsell-items').html(upsel_markup);
        $('.sidebarcart-upsell').show();

      } else {
        $('.sidebarcart-upsell').hide();
      }
    });
...
// Cart Drawer Upsell CTA
document.addEventListener('page:loaded', function() {
  $(document).on('click', '.cart-drawer-upsell-cta', function(e) {
    ...
    $.ajax({
      ...
      url: '/cart/add.js',
      dataType: 'json',
      data: $('form.ProductCartUpsell[action="/cart/add"]').serialize(),
      success: function() {
        // Cart refresh
...
/* Cart Upsell - Hafiz Adnan Hussain - END */

/* 2633 - Top Nav Redesign/IA Restructure - By: Hafiz Adnan Hussain - START */
...
jQuery(document).ready(function(){
  ...
  jQuery(document).on('click', '.bws-account-header-link', function(evt){
    evt.preventDefault();
    jQuery('.bws-login-overlay').show();
    jQuery('.bws-login-popup').show();
  });
  jQuery(document).on('click', '#btn-login-modal-close', function(evt){
    evt.preventDefault();
    jQuery('.bws-login-overlay').hide();
    jQuery('.bws-login-popup').hide();
  });

  jQuery('.bws-login-popup #customer_login').submit(function(evt){

    $(this).find('button[type="submit"]').addClass('btn--loading').prop('disabled', true);
    var data = {
      'customer[email]': $(this).find('#CustomerEmailPopup').val(),
      'customer[password]': $(this).find('#CustomerPasswordPopup').val(),
    ...
    jQuery.ajax({
      ...
      success: function success(data) {
        if (data.indexOf('class="errors"') !== -1) {
          // invalid password - show a message to the user
          if($('.errors', $(data))[0]) {
            $('.bws-login-popup-errors').html($('.errors', $(data))[0].outerHTML);
          } else {
            $('.bws-login-popup-errors').html('<div class="errors"><ul><li>There is an error, please try again!</li></ul></div>');
          }
          jQuery('.bws-login-popup #customer_login').find('button[type="submit"]').removeClass('btn--loading').removeAttr('disabled');
        } else {
          jQuery('.bws-login-popup #customer_login').find('button[type="submit"]').removeClass('btn--loading').text('Success!');
